...
import { faBan, faCancel, faCheck, faCheckCircle, faCheckDouble, faEye, faXmark, faXmarkCircle } from "@fortawesome/free-solid-svg-icons"
...
import { Payment, User, MutationResult, Project } from "../../../types"
...
function Payments() {
   ...
   const app = useSelector((state: { app: any }) => state.app.app)
   ...
   const handleSetStatus = async (status: string, payment: Payment) => {
      if (status === "REJECTED") {
         ...
         await createActivityMutation({
            userRole: auth.type,
            entry: `${ auth.username }-reject-payment`,
            module: "PAYMENTS",
            category: "OTHERS",
         ...
         if (rejectPayment?.data!?.id) {
            toast(
               <div className="flex justify-center items-center gap-x-3">
                  ...
                  <h1 className="text-white font-grandview-bold">Payment Rejected!</h1>
               </div>,
               {
         ...
         if (acceptPayment?.data!?.id) {
            toast(
               <div className="flex justify-center items-center gap-x-3">
                  <FontAwesomeIcon className="text-white" icon={faCheckCircle} size="lg" fixedWidth />
                  <h1 className="text-white font-grandview-bold">Payment accepted!</h1>
               </div>,
               {
                  toastId: "register-failed-toast",
                  theme: "colored",
         ...
         await createActivityMutation({
            userRole: auth.type,
            entry: `${ auth.username }-accept-payment`,
            module: "PAYMENTS",
            category: "OTHERS",
   ...
   return (
      <Fragment>
         ...
         <main className="grow flex flex-col justify-start items-start gap-y-5 w-full h-full px-20 py-10">
            <HeaderGroup text="Payments" link="/accounting/dashboard" />
            ...
            <div className="flex flex-row justify-start items-center gap-x-5">
               ...
               <AddButton text="Add New Payment" onClick={() => navigate("/accounting/payments/create")} />
            </div>
