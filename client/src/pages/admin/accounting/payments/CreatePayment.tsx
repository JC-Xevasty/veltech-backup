...
import type { FieldTypes, MutationResult, OutletContext, Payment, ProjectMilestone, ReduxState, UploadedFile, User } from "../../../../types"
  ...
  const handleCreate: SubmitHandler<FieldValues> = async (fields) => {
    ...
    if (isFileUploaded) {
      ...
      await createActivityMutation({
        userRole: user.type,
        entry: `${ user.username }-create-payment`,
        module: "CREATE-PAYMENT",
        category: "OTHERS",
      ...
      if (createPayment?.data!?.id) {
        ...
        navigate("/admin/accounting/payments", {
          replace: true
        })
      }
      ...
      <main className={`${offset}`}>
        <div className='flex justify-between px-5 items-center'>
          <div className='flex items-center w-full justify-between'>
            <HeaderGroup text="Add Payment" link="/admin/accounting/payments" />
          </div>
        </div>

        <form className="flex flex-col gap-y-5 p-5" onSubmit={handleSubmit(handleCreate)}>
          ...
          <div className="flex gap-x-5">
            ...
            <button onClick={() => { reset(); navigate("/admin/accounting/payments") }} className=" w-fit transition-all ease-in-out duration-300 text-black tracking-wide rounded-sm bg-[#EBEBEB] px-3 py-1.5 hover:scale-105" type="button">Cancel</button>
          </div>
        </form>
      </main>
    </>
